...
function BoardSizeInput({ rows, cols, onApply }) {
  ...
  return (
    <div className="board-size-input">
      ...
      &nbsp;
      <button onClick={() => onApply(inputRows, inputCols)}>Resize</button>
    </div>
  );
}
...
export default function Game() {
  ...
  function handleApplySize(newRows, newCols) {
    // Map current board into new dimensions, anchored to top-left.
    // Pieces within the new bounds are preserved; cells outside are cropped;
    // new cells (when expanding) start empty.
    ...
    for (let r = 0; r < Math.min(rows, newRows); r++) {
      for (let c = 0; c < Math.min(cols, newCols); c++) {
        newSquares[r * newCols + c] = currentSquares[r * cols + c];
      }
    }
    setRows(newRows);
    setCols(newCols);
